...
spp <- read.csv('data/new_york_fishes.csv', header = TRUE, row.names = 1)

spp <- read.csv(paste(fn, '.csv', sep = ''),
                header = TRUE,
                row.names = 1)
...
hist(
  ...
  right = FALSE,
  xlim = c(0, nws),
  xlab = 'Number of Watersheds per Species',
  main = paste(
...
hist(
  ...
  right = FALSE,
  xlim = c(0, highSp),
  xlab = 'Number of Species per Watershed',
  main = paste(
